...
- Markdown for agents through [nuxt-agent-discovery](https://github.com/benjamincanac/nuxt-agent-discovery): content negotiation on every documentation page, raw markdown mirrors (`/raw/**`), `llms.txt` / `llms-full.txt`, `sitemap.md`, `/openapi.json`, `/.well-known/api-catalog`, an MCP server (`/mcp`) with its server card, and Agent Skills discovery (`/.well-known/skills/` and `/.well-known/agent-skills/`).
...
Drop skills into a `skills/` directory at the app root. [nuxt-agent-discovery](https://github.com/benjamincanac/nuxt-agent-discovery) serves the v0.1 catalog and skill files under `/.well-known/skills/`; this layer also publishes the [Agent Skills Discovery RFC](https://github.com/cloudflare/agent-skills-discovery-rfc) v0.2 catalog at `/.well-known/agent-skills/index.json`.

Users install the skills with:
